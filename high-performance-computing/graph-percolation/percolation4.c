...
#pragma omp parallel default(shared) private(myseed)
  ...
  /--------------------------------*/
  #pragma omp parallel for num_threads(2) default(shared) private(Pcon, igraph, i, j, xi, lclus, nclus, avlclus, avnclus) ordered   
  for (ip=0;ip<Np;ip++) {
    ...
    / Loop over graphs  /
    /------------------*/
    #pragma omp parallel for num_threads(2) default(shared) private(Ncon, Lcon, i, j, xi, lclus, nclus) reduction(+:avnclus, avlclus)
